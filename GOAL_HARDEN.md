# Goal: Audit and harden `go-wire`

## Mission

Perform an evidence-driven security, correctness, interoperability, and
resource-safety audit of every wire format supported by `go-wire`, then
...
- The repository's `GOAL.md`, `AGENTS.md`, format matrix, API docs, fixtures,
...
- Identify dependency defaults that conflict with `go-wire` guarantees and
