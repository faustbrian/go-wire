...
- `ErrorKindParse`, `ErrorKindValidation`, `ErrorKindUnsupported`,
  `ErrorKindEnvelope`, `ErrorKindFault`, and `ErrorKindWrite` are stable
  classifications.
- `ErrParse`, `ErrValidation`, `ErrUnsupportedFormat`, `ErrEnvelope`, and
  `ErrSOAPFault`, and `ErrWrite` are sentinels for `errors.Is`.
...
- `EncodeOptions` contains XML indentation for typed Header and Body values.
