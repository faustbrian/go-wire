...
- `go-jsonapi` carries JSON:API feature, conformance, extension/profile,
  recommendation, and threat-model documentation.
- `go-jsonrpc` carries protocol conformance and middleware documentation.
- `go-queue` carries backend, delivery, lifecycle, failure, and integration
  documentation plus a live-backend integration workflow. Its fork provenance
  requires detailed third-party notices.
- `go-wire` carries format, dependency, and audit-evidence documentation.
- `go-tabular` carries format and ingest-limit documentation. It uses
