...
## Can `Marshal` add headers or bodies from structs?

Encode a struct with `xmlwire.Encode`, then pass the resulting fragment to
`soap.Marshal`. This keeps SOAP envelope behavior separate from business DTOs.
