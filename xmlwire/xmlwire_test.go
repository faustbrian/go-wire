...
func TestDecodeOptionsAndReaderFailures(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "nil target", reader: strings.NewReader(`<root/>`), target: nil, kind: wire.ErrValidation},
		{name: "non-pointer", reader: strings.NewReader(`<root/>`), target: shipment{}, kind: wire.ErrValidation},
		...
		{name: "too large", reader: strings.NewReader(`<root/>`), target: &shipment{}, options: xmlwire.DecodeOptions{MaxBytes: 3}, kind: wire.ErrValidation},
