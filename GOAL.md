...
Build an open source Go package for structured wire-format handling that starts
with JSON, XML, and SOAP as first-class concerns.
...
The first version is intentionally scoped to JSON, XML, and SOAP so the
package can become excellent at those three concerns before expanding into a
generic “supports every format” bucket.
...
### 1. Excellent JSON/XML/SOAP Interop
...
- understand what JSON/XML/SOAP support is actually provided
...
- JSON/XML/SOAP support is production-credible
...
- Do not expand to YAML, TOML, or MessagePack before JSON/XML/SOAP is solid.
