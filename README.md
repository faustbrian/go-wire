...
go-wire requires Go 1.25.12 or newer.
...
Outbound JSON and XML can be returned as bytes with `Encode` or written
directly to an `io.Writer` with `EncodeWriter`. SOAP supports the same typed
flow for both the optional Header and Body:
...
rewriting them. Faults have matching byte and writer APIs. The package does not
